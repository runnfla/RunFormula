...
//  See LICENSE file in the project root or copy at https://opensource.org
...
procedure RunFlaError(ErrCode:TRunFlaErrCode);
...
var Buf : array of string;
    ...
    ClsList, SubrList, DefList : TMemList;   // closing Tokens, subroutines, defines
...
begin
  with PToken(Pnt)^ do begin
    Context.FlaPos:=Source;
    case Tag of
      TagValue   : Result:=@ValRec;
      TagVar     : begin
                     for i:=Context.VarPool.Count-1 downto 0 do begin
                       lst:=MemListGet(Context.VarPool, i);
                       idx:=MemListFind(lst^, @Text);
                       if idx>=0 then break;
                     end;
                     if idx<0 then begin
                       if PToken(Pnt+Size)^.Tag<>TagAssign then begin
                         if Context.RunFlaVar=nil then raise EError.Create(VarUnknown);
                         Result:=Vrt2Val(Context.RunFlaVar(string(@Text), flg));
                         if flg then ValCopy(Result, CreateNewVar(Pnt));
                       end else Result:=CreateNewVar(Pnt);
                     end else Result:=@PVariable(MemListGet(lst^, idx))^.Val;
                   end;
      TagArray   : raise EError.Create(Unsupport);
  ...
  begin
    if Err<>OK then begin
      ...
      if Error.Code=OK then begin
        Error.Code:=Err;
        if P=nil then Error.Position:=Context.FlaPos else Error.Position:=PToken(P)^.Source;
      end;
    end;
  end;
...
begin
  ...
  try
    ...
    on E:EFlaReturn do FillError(OpIncorrect);
    on E:EFlaBreak do FillError(OpIncorrect);
    on E:EFlaContinue do FillError(OpIncorrect);
    on E:EOverflow do FillError(Overflow);
...
procedure RunFlaError(ErrCode:TRunFlaErrCode);            //DONE -oMain -cRev.2026.03.28: Proc RunFlaError
begin
...
begin
  try
    i:=MemListFind(FuncList, PByte(Name));
    if i<0 then begin
